...
Для обеспечения прозрачности взаимодействия между серверной частью и клиентом была разработана комплексная документация REST API с использованием библиотеки **SpringDoc OpenAPI**. Это решение позволяет автоматически генерировать спецификацию на основе кода контроллеров и аннотаций, гарантируя, что документация всегда остается актуальной и соответствует текущему состоянию системы. Визуализация интерфейса реализована через **Swagger UI**, который предоставляет интерактивную среду для разработчиков: каждый эндпоинт сопровождается подробным описанием, примерами входных данных (DTO) и возможных ответов сервера, а также позволяет отправлять тестовые запросы непосредственно из браузера без необходимости написания дополнительного кода.

Особое внимание при разработке документации было уделено вопросам безопасности и удобству навигации. В конфигурации OpenAPI реализована поддержка схемы авторизации на основе **JWT-токенов**, что добавляет кнопку авторизации в интерфейс Swagger. Это позволяет тестировщикам и фронтенд-разработчикам выполнять запросы к защищенным ресурсам, таким как оформление заказов или чат с AI-ассистентом, предварительно введя токен доступа. Все методы API логически сгруппированы по функциональным модулям — *аутентификация, каталог, заказы и аналитика* — с помощью аннотаций, что структурирует информацию и упрощает поиск необходимых операций.

Кроме того, разработанная документация служит основой для интеграции с внешними платформами тестирования. Генерируемая спецификация в формате JSON, доступная по специальному маршруту, полностью совместима со стандартом **OpenAPI 3.0** и может быть импортирована в инструменты вроде **Postman**.

Этот файл позволяет автоматически создавать готовые коллекции запросов, избавляя от ручной настройки параметров и заголовков, что существенно ускоряет процесс отладки и интеграционного тестирования разработанного программного средства.


Полную документацию API можно скачать или просмотреть по ссылке:
[api-docs.json](docs%2Fapi-docs.json)
...
В ходе оценки архитектурного качества кода было установлено, что применение методологии **Domain-Driven Design (DDD)** позволило достичь высоких показателей метрик **«High Cohesion»** (высокая связность) и **«Low Coupling»** (слабая связанность). Разделение приложения на слои `api`, `application`, `domain` и `infrastructure` обеспечило четкую изоляцию ответственности. В частности, использование шаблона **«Богатая модель» (Rich Domain Model)** в модулях `orders` и `reviews` позволило инкапсулировать бизнес-логику и правила валидации (например, расчет стоимости заказа или проверку рейтинга) непосредственно внутри сущностей. Это снизило сложность сервисного слоя (`OrderService`, `ReviewService`), превратив сервисы из тяжеловесных в легковесные, что значительно упрощает тестирование и дальнейшее сопровождение кода.

Для оценки метрики **объема кода и читаемости** было проанализировано влияние библиотеки **Lombok**. Использование аннотаций `@Data`, `@Builder` и `@RequiredArgsConstructor` позволило сократить объем шаблонного кода (getter/setter, конструкторы, builder-паттерны) примерно на 40–50% по сравнению с классическим стилем Java. Это положительно сказалось на **индексе поддерживаемости**, так как код стал более лаконичным и сфокусированным на бизнес-правилах, а не на инфраструктурных конструкциях языка. Также строгое разделение моделей данных на JPA-сущности и DTO (Data Transfer Objects) предотвратило утечку абстракций базы данных в API-слой, что повышает безопасность и гибкость контрактов интерфейса.

Оценка **метрик производительности** проводилась на основе логов запуска и времени отклика. Время холодного старта приложения составило **3,374 секунды**, что является отличным показателем для Spring Boot приложения с подключенными драйверами MySQL и Elasticsearch. Внедрение полнотекстового поиска через **Elasticsearch** вместо стандартных SQL-запросов с оператором `LIKE` позволило свести временную сложность поиска к логарифмической, обеспечивая мгновенный отклик даже при росте объема каталога. Конфликты ресурсов (например, портов) были устранены на этапе конфигурации, а использование Docker-контейнеризации гарантирует предсказуемость метрик стабильности в различных средах развертывания.

Таким образом, программное средство демонстрирует высокое качество кода, соответствующее промышленным стандартам разработки на **Spring Boot 3**. Архитектура устойчива к изменениям, код обладает низкой когнитивной сложностью для разработчика, а выбранные технические решения (кеширование на уровне БД, индексы Elastic, реактивная работа UI через Swagger) обеспечивают высокую производительность системы.
